    ...
    return (
      <div className="flex items-center space-x-1">
        {isSortable ? (
            ...
            >
                <span>{title}</span>
                {sortDirection === 'asc' && <ChevronUp className="ml-2 h-3.5 w-3.5 text-primary" />}
                {sortDirection === 'desc' && <ChevronDown className="ml-2 h-3.5 w-3.5 text-primary" />}
            </Button>
        ) : (
                                ...
                                filterContent={
                                    <div className="space-y-3">
                                        <div className="space-y-2">
                                            {OVERTIME_TYPES.filter(t => availableTypes.includes(t.id)).length > 0 ? (
                                                OVERTIME_TYPES.filter(t => availableTypes.includes(t.id)).map((type) => (
                                                ...
                                                <div className="text-xs text-muted-foreground py-2">No types available</div>
                                            )}
                                        </div>
                                        <Button 
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center space-x-2.5">
                                                    <Checkbox 
                                                        ...
                                                        onCheckedChange={() => onFilterChange({ ...filters, changeFilter: 'all' })}
                                                    />
                                                ...
                                                {availableChangeOptions.hasPositive && (
                                                    <div className="flex items-center space-x-2.5">
                                                        <Checkbox 
                                                            ...
                                                            onCheckedChange={() => onFilterChange({ ...filters, changeFilter: 'positive' })}
                                                        />
                                                        <Label htmlFor="chg-pos" className="text-green-600">Added (+)</Label>
                                                    </div>
                                                )}
                                                ...
                                                {availableChangeOptions.hasNegative && (
                                                    <div className="flex items-center space-x-2.5">
                                                        <Checkbox 
                                                            ...
                                                            onCheckedChange={() => onFilterChange({ ...filters, changeFilter: 'negative' })}
                                                        />
                                                        <Label htmlFor="chg-neg" className="text-destructive">Deducted (-)</Label>
                                                    </div>
                                                )}

                                                {!availableChangeOptions.hasPositive && !availableChangeOptions.hasNegative && (
                    ...
                    {sortedRecords.length === 0 ? (
                        ...
                        sortedRecords.map((record, i) => (
                            ...
                            >
                                ...
                                <TableCell>
                                    <Badge variant="secondary" className="font-normal bg-secondary text-secondary-foreground border-border hover:bg-secondary/80">
                                        {OVERTIME_TYPES.find(t => t.id === record.type)?.label || record.type}
                                    </Badge>
                                </TableCell>
                                <TableCell className={cn(
                                    "text-right font-mono font-medium tabular-nums",
                                    record.hours > 0 ? "text-green-600 dark:text-green-500" : "text-destructive"
                                )}>
        ...
        <div className="space-y-6 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
            <div className="flex flex-col gap-1 mb-2">
                <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className="font-mono text-sm px-3 py-1 bg-background">
                        Total Balance: <span className={cn("ml-2 font-bold", totalBalance < 0 ? "text-destructive" : "text-primary")}>{totalBalance.toFixed(2)} h</span>
                    </Badge>
                </div>
            </div>

            <Tabs value={mode} onValueChange={(v) => setMode(v as 'add' | 'subtract')} className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="add" className="data-[state=active]:bg-green-500/10 data-[state=active]:text-green-700 dark:data-[state=active]:text-green-400 transition-all">
                        <TrendingUp className="w-4 h-4 mr-2" /> Add Hours
                    </TabsTrigger>
                    <TabsTrigger value="subtract" className="data-[state=active]:bg-red-500/10 data-[state=active]:text-red-700 dark:data-[state=active]:text-red-400 transition-all">
                        ...
                        <PopoverTrigger asChild>
                            <Button
                                ...
                                className={cn(
                                    ...
                                    !date && "text-muted-foreground border-dashed border-muted-foreground/50"
                                )}
                            >
            ...
            <div className="grid grid-cols-1 gap-3">
                {date && (
                    <div className="p-3 rounded-lg border bg-muted/30 flex justify-between items-center text-sm">
                        <div className="flex items-center gap-2">
                             ...
                             <span className="text-muted-foreground">Day Balance ({format(date, 'dd.MM')}):</span>
                        </div>
                        <div className="flex items-center gap-3 font-mono">
                            ...
                            <span className={cn(
                                "font-bold text-base",
                                predictedDayBalance < 0 ? "text-destructive" : "text-green-600"
                            )}>
                                {predictedDayBalance > 0 ? '+' : ''}{predictedDayBalance.toFixed(2)} h
                            </span>
                        </div>
                    </div>
                )}
                ...
                <div className={cn(
                    ...
                    mode === 'add' ? "bg-green-500/5 border-green-500/20" : "bg-red-500/5 border-red-500/20"
                )}>
                    <span className="text-muted-foreground font-medium">New Total Balance (Year)</span>
                    <div className="flex items-center gap-2 font-mono font-bold">
                        <span className="text-foreground/60">{totalBalance.toFixed(2)}</span>
                        <ArrowRight className="w-3 h-3 text-muted-foreground" />
                        <span className={predictedTotalBalance < 0 ? "text-destructive" : "text-green-600"}>
            ...
            >
                ...
                {isSaveDisabled && !submitting && !date ? 'Select Date to Save' : (mode === 'subtract' ? 'Confirm Deduction' : 'Confirm Addition')}
            </Button>
        </div>
    );
}
...
export default function OvertimePage() {
  ...
  const { data, loading } = useQuery<OvertimeSummaryData>(GET_OVERTIME_SUMMARY, {
    variables: {
  ...
  const baseRecords = useMemo(() => {
      ...
      return historyRecords.filter(r => r.note?.toLowerCase().includes(lowerSearch));
  ...
  const filteredRecords = useMemo(() => {
      return baseRecords.filter(r => {
          // Type Filter
          if (filters.selectedTypes.length > 0 && !filters.selectedTypes.includes(r.type)) {
              return false;
          }
          // Change Filter
  ...
  if (authLoading) return <div className="flex h-screen items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>;

  return (
    <div className="container mx-auto py-8 px-4 max-w-7xl space-y-8 animate-in fade-in-0 duration-500">
      
      {/* 1. Header (Len text) */}
      <div className="flex flex-col space-y-1 pb-2">
         <h1 className="text-3xl font-bold tracking-tight text-foreground">Overtime</h1>
         <p className="text-muted-foreground">Manage balances and corrections.</p>
      </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex flex-col gap-1">
                                            <span className={cn("font-medium transition-colors", isSelected ? "text-primary" : "text-foreground")}>
                                                {type.label}
                                            </span>
                                        </div>
                                        <div className={cn(
                                            "text-lg font-bold tabular-nums",
                                            balance < 0 ? "text-destructive" : "text-green-600 dark:text-green-500"
                                        )}>
                                            {balance.toFixed(2)} <span className="text-xs font-normal text-muted-foreground">h</span>
                                        </div>
                                    </button>
                                );
                            })

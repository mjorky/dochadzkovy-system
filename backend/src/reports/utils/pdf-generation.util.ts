...
export async function generateWorkReportPDF(data: WorkReport, employeeName: string, month: number, year: number): Promise<Buffer> {
  let browser;
  try {
    ...
    const content = getWorkReportHTML(data, employeeName, month, year);

...
export default function WorkReportPage() {
    ...
    // --- API Hooks ---
    const { data: employeesData, loading: employeesLoading } = useQuery<EmployeesCatalogData>(GET_EMPLOYEES_CATALOG);

    const { data: summaryData, refetch: refetchSummary } = useQuery(GET_WORK_REPORT_DATA, {
        variables: {
    ...
    const [getProjectReport, { data: projectReportData, loading: projectReportLoading }] = useLazyQuery(GET_WORK_LIST_REPORT);

    const [generateWorkListPdf, { loading: workListPdfLoading, data: workListPdfData }] = useLazyQuery(GET_WORK_LIST_REPORT_PDF, { fetchPolicy: 'network-only' });
    ...
    const handleDownloadPdf = () => {
        ...
        link.href = url;
        const empName = employeesData?.employees.find(e => e.id === selectedEmployee)?.fullName || 'report';
    ...
    const projectTotals = projectItems.reduce((acc: any, item: any) => ({
    ...
    return (
        <div className="w-full px-4 md:px-8 py-8 space-y-8">
            ...
            <div className="flex flex-col gap-2">
                <h1 className="text-3xl font-bold tracking-tight">Monthly Work Report</h1>
                <p className="text-muted-foreground">
                    Generate, view, and export monthly work reports with project details.
                </p>
            </div>
                ...
                <div className="lg:col-span-2 xl:col-span-3 space-y-6">
                    ...
                    {summaryData?.getWorkReportData && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <Card>
                                <CardContent className="pt-6 text-center">
                                    <div className="text-2xl font-bold text-primary">{summaryData.getWorkReportData.totalHours.toFixed(1)}</div>
                                    <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Hours</div>
                                </CardContent>
                            </Card>
                            <Card>
                                <CardContent className="pt-6 text-center">
                                    <div className="text-2xl font-bold text-orange-600">{summaryData.getWorkReportData.weekendWorkHours.toFixed(1)}</div>
                                    <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Weekend (h)</div>
                                </CardContent>
                            </Card>
                            <Card>
                                <CardContent className="pt-6 text-center">
                                    <div className="text-2xl font-bold text-blue-600">{summaryData.getWorkReportData.holidayWorkHours.toFixed(1)}</div>
                                    <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Holiday (h)</div>
                                </CardContent>
                            </Card>
                        </div>
                    )}
                    ...
                    <Card className="h-full">
                        <CardHeader>
                            ...
                            <CardDescription>Detailed list of hours allocated to projects.</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-muted/50">
                                            <TableHead className="whitespace-nowrap">Project No.</TableHead>
                                            <TableHead className="whitespace-nowrap min-w-[200px]">Project Name</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">Prod SK</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">NonProd SK</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">Prod Z</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">NonProd Z</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">Prod 70</TableHead>
                                            <TableHead className="text-center whitespace-nowrap">KM</TableHead>
                                        ...
                                        {projectReportLoading ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={9} className="h-24 text-center text-muted-foreground">
                                                    {selectedEmployee ? 'No data found for this period.' : 'Select an employee to view details.'}
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            <>
                                                {projectItems.map((item: any, idx: number) => (
                                                    <TableRow key={idx}>
                                                        <TableCell className="font-mono text-xs whitespace-nowrap">{item.projectNumber}</TableCell>
                                                        <TableCell className="max-w-[300px] truncate" title={item.projectName}>
                                                            {item.projectName}
                                                        </TableCell>
                                                        <TableCell className="text-center font-medium">{item.productiveHours > 0 ? item.productiveHours.toFixed(2) : '0'}</TableCell>
                                                        <TableCell className="text-center text-muted-foreground">{item.nonProductiveHours > 0 ? item.nonProductiveHours.toFixed(2) : '0'}</TableCell>
                                                        <TableCell className="text-center text-muted-foreground">{item.productiveOutSkCzHours > 0 ? item.productiveOutSkCzHours.toFixed(2) : '0'}</TableCell>
                                                        <TableCell className="text-center text-muted-foreground">{item.nonProductiveZHours > 0 ? item.nonProductiveZHours.toFixed(2) : '0'}</TableCell>
                                                        <TableCell className="text-center text-muted-foreground">{item.productive70Hours > 0 ? item.productive70Hours.toFixed(2) : '0'}</TableCell>
                                                        <TableCell className="text-center">{item.travelKm}</TableCell>
                                                        <TableCell className="text-xs whitespace-nowrap">{item.projectManagerName}</TableCell>
                                                    </TableRow>
                                                ))}
                                                <TableRow className="bg-primary/5 font-bold">
                                                    <TableCell colSpan={2}>Total</TableCell>
                                                    <TableCell className="text-center">{projectTotals.prodSK.toFixed(2)}</TableCell>
                                                    <TableCell className="text-center">{projectTotals.nonProdSK.toFixed(2)}</TableCell>
                                                    <TableCell className="text-center">{projectTotals.prodZ.toFixed(2)}</TableCell>
                                                    <TableCell className="text-center">{projectTotals.nonProdZ.toFixed(2)}</TableCell>
                                                    <TableCell className="text-center">{projectTotals.prod70.toFixed(2)}</TableCell>
                                                    <TableCell className="text-center">{projectTotals.travel}</TableCell>
                                                    <TableCell></TableCell>
                                                </TableRow>
                                            </>
                                        )}

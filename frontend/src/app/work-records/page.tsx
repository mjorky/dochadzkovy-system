...
import { Loader2, XCircle, Calendar as CalendarIcon, Plus, Download } from 'lucide-react';
...
export default function WorkRecordsPage() {
    ...
    const handleFetchMore = useCallback(async () => {
        if (!recordsData?.getWorkRecords.hasMore || isFetchingMore || recordsLoading) return;
        ...
        if (result.data?.getWorkRecords.records) {
            setAllRecords((prev) => [...prev, ...result.data.getWorkRecords.records]);
    ...
    }, [recordsData?.getWorkRecords.hasMore, isFetchingMore, recordsLoading, fetchMore, selectedEmployeeId, filters.fromDate, filters.toDate, offset]);
    ...
    const handleFilterChange = useCallback((newFilters: WorkRecordsFilterState) => {
    ...
    const handleAddEntry = () => { setSelectedRecord(null); setCreateDialogOpen(true); };
    const handleEdit = (record: WorkRecord) => { setSelectedRecord(record); setEditDialogOpen(true); };
    const handleDelete = (record: WorkRecord) => { setRecordToDelete(record); setDeleteDialogOpen(true); };
    const handleCopy = (record: WorkRecord) => { setCopyFromRecord(record); setCreateDialogOpen(true); };

    const handleExportCSV = () => {
        try {
            const currentEmployee = employeesData?.employees.find(emp => emp.id === selectedEmployeeId);
  ...
  const matchesProject = (record: WorkRecord) => {
    ...
    const projectNumber = projectsData?.getActiveProjects.find(p => filters.selectedProjects.includes(p.id))?.number;
  ...
  const matchesAbsence = (record: WorkRecord) => {
    ...
    const absenceAlias = absenceTypesData?.getAbsenceTypes.find(a => filters.selectedAbsenceTypes.includes(a.id))?.alias;
  ...
  const matchesProductivity = (record: WorkRecord) => {
    ...
    const prodType = productivityTypesData?.getProductivityTypes.find(p => filters.selectedProductivityTypes.includes(p.id))?.hourType;
  ...
  const matchesWorkType = (record: WorkRecord) => {
    ...
    const workType = workTypesData?.getWorkTypes.find(w => filters.selectedWorkTypes.includes(w.id))?.hourType;
  ...
  const matchesSearch = (record: WorkRecord) => {
    ...
    return record.description?.toLowerCase().includes(filters.searchText.toLowerCase());
  };
  ...
  const availableProjects = useMemo(() => {
    const relevantRecords = baseRecords.filter(r => 
        matchesAbsence(r) && matchesProductivity(r) && matchesWorkType(r) && matchesLock(r) && matchesTrip(r)
    );
    const uniqueNumbers = new Set(relevantRecords.map(r => r.project).filter(Boolean));
    return (projectsData?.getActiveProjects || []).filter(p => uniqueNumbers.has(p.number));
  }, [baseRecords, filters, projectsData]);

  const availableAbsenceTypes = useMemo(() => {
    const relevantRecords = baseRecords.filter(r => 
        matchesProject(r) && matchesProductivity(r) && matchesWorkType(r) && matchesLock(r) && matchesTrip(r)
    );
    const uniqueAliases = new Set(relevantRecords.map(r => r.absenceType).filter(Boolean));
    return (absenceTypesData?.getAbsenceTypes || []).filter(a => uniqueAliases.has(a.alias));
  }, [baseRecords, filters, absenceTypesData]);

  const availableProductivityTypes = useMemo(() => {
    const relevantRecords = baseRecords.filter(r => 
        matchesProject(r) && matchesAbsence(r) && matchesWorkType(r) && matchesLock(r) && matchesTrip(r)
    );
    const uniqueTypes = new Set(relevantRecords.map(r => r.productivityType).filter(Boolean));
    return (productivityTypesData?.getProductivityTypes || []).filter(p => uniqueTypes.has(p.hourType));
  }, [baseRecords, filters, productivityTypesData]);

  const availableWorkTypes = useMemo(() => {
    const relevantRecords = baseRecords.filter(r => 
        matchesProject(r) && matchesAbsence(r) && matchesProductivity(r) && matchesLock(r) && matchesTrip(r)
    );
    const uniqueTypes = new Set(relevantRecords.map(r => r.workType).filter(Boolean));
    return (workTypesData?.getWorkTypes || []).filter(w => uniqueTypes.has(w.hourType));
  }, [baseRecords, filters, workTypesData]);

  // NOVÉ: VÝPOČET DOSTUPNÝCH LOCK STATUSOV
  const availableLockOptions = useMemo(() => {
      const relevantRecords = baseRecords.filter(r => 
          matchesProject(r) && matchesAbsence(r) && matchesProductivity(r) && matchesWorkType(r) && matchesTrip(r)
      );
      return {
  ...
  const availableTripOptions = useMemo(() => {
      const relevantRecords = baseRecords.filter(r => 
          matchesProject(r) && matchesAbsence(r) && matchesProductivity(r) && matchesWorkType(r) && matchesLock(r)
      );
      return {
  ...
  const filteredRecords = useMemo(() => {
    return baseRecords.filter(r => 
        matchesProject(r) && matchesAbsence(r) && matchesProductivity(r) && 
        matchesWorkType(r) && matchesLock(r) && matchesTrip(r)
    );
  }, [baseRecords, filters, projectsData, absenceTypesData, productivityTypesData, workTypesData]);
     ...
     return (
        <div className="flex items-center justify-center min-h-screen bg-background">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground animate-pulse">Loading work records...</p>
          </div>
        </div>
      );
  }
          ...
          <Card className="max-w-md border-destructive/50 bg-destructive/5 shadow-lg">
            <CardContent className="flex flex-col items-center gap-4 text-center p-8">
              ...
              <CardHeader className="p-0">
                ...
                <CardDescription>{recordsError.message || 'An unexpected error occurred'}</CardDescription>
              </CardHeader>
              <Button onClick={() => refetchRecords()} variant="outline" className="border-destructive/20 hover:bg-destructive/10">Retry</Button>
            </CardContent>
          </Card>
        </div>
      );
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto space-y-6">
       {/* ... (Breadcrumb, Header, EmployeeSelector, DatePicker sú bez zmeny) ... */}
       <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem><BreadcrumbLink href="/" className="hover:text-foreground transition-colors">Home</BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Work Records</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h1 className="text-3xl font-bold tracking-tight text-foreground">Work Records</h1>
           <p className="text-muted-foreground mt-1">Manage and track work entries.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-end gap-4 p-5 rounded-xl border border-border bg-card shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
          <div className="flex flex-col gap-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal border-input hover:bg-accent hover:text-accent-foreground", !filters.fromDate && "text-muted-foreground")}>
              ...
              <PopoverContent className="w-auto p-0 animate-in fade-in-0 zoom-in-95" align="start">
                <Calendar mode="single" selected={filters.fromDate || undefined} onSelect={(date) => handleFilterChange({ ...filters, fromDate: date || null, showWholeMonth: false })} initialFocus />
              </PopoverContent>
            </Popover>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal border-input hover:bg-accent hover:text-accent-foreground", !filters.toDate && "text-muted-foreground")}>
                  <CalendarIcon className="mr-2 h-4 w-4 opacity-50" />
                  {filters.toDate ? format(filters.toDate, "PPP") : <span>Pick a date</span>}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 animate-in fade-in-0 zoom-in-95" align="start">
                <Calendar mode="single" selected={filters.toDate || undefined} onSelect={(date) => handleFilterChange({ ...filters, toDate: date || null, showWholeMonth: false })} initialFocus />
              </PopoverContent>
            </Popover>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 h-10 pb-1 px-2">
          ...
          />
          <Label htmlFor="wholeMonth" className="cursor-pointer text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Whole Month</Label>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          <Button onClick={handleAddEntry} className="gap-2 shadow-sm bg-primary text-primary-foreground hover:bg-primary/90">
      ...
      {filteredRecords.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 py-16 text-center border-2 border-dashed border-muted rounded-xl bg-muted/5">
          ...
          <div className="max-w-xs">
            <h2 className="text-lg font-semibold text-foreground mb-1">No records found</h2>
            <p className="text-sm text-muted-foreground">We couldn't find any work records for this date range or filter selection.</p>
          </div>
          {(filters.selectedProjects.length > 0 || filters.searchText || filters.selectedAbsenceTypes.length > 0) && (
            <Button variant="link" onClick={() => handleFilterChange({ ...filters, searchText: '', selectedProjects: [], selectedAbsenceTypes: [], selectedProductivityTypes: [], selectedWorkTypes: [], lockStatus: 'all', tripFlag: 'all' })} className="text-primary">
              Clear all filters
            </Button>
          )}
        </div>
        ...
        <>
          <div className="flex justify-end items-center mb-2">
             <span className="text-xs font-medium text-muted-foreground bg-secondary/50 px-2 py-1 rounded-md">
                Showing {filteredRecords.length} of {recordsData?.getWorkRecords.totalCount || allRecords.length} records
             </span>
          </div>
          ...
          <div ref={observerTarget} className="py-8 text-center">
            {recordsData?.getWorkRecords.hasMore && isFetchingMore && (
              <div className="flex items-center justify-center gap-2 p-2 rounded-full bg-secondary/30 inline-flex px-4">
                ...
                <span className="text-xs font-medium text-muted-foreground">Loading more records...</span>
              </div>
            )}
          </div>
        </>
      ...
      <WorkRecordDialog
        ...
        onOpenChange={(open) => { setCreateDialogOpen(open); if (!open) setCopyFromRecord(null); }}
        mode="create"
        employeeId={parseInt(selectedEmployeeId, 10)}

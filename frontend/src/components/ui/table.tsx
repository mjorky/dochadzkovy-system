...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("sticky top-0 bg-muted/90 backdrop-blur-sm z-10 shadow-sm [&_tr]:border-b", className)}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-muted/60 active:bg-muted data-[state=selected]:bg-muted border-b border-border transition-colors",
        className
      )}
      {...props}
    />
